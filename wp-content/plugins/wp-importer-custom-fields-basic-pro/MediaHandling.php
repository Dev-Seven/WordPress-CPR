	...
	public function mediaReport(){
		...
		foreach( $list_of_images as $list_key => $list_val )
		{
			if(!empty($list_val['hash_key'])){
				$file_name = $wpdb->get_results("select file_name from {$wpdb->prefix}smackcf_file_events where hash_key = '{$list_val['hash_key']}'",ARRAY_A);
			}
			...
			$number_of_images = $wpdb->get_results("select image_url from {$wpdb->prefix}ultimate_cf_importer_media where hash_key = '{$list_val['hash_key']}' and image_type = '{$image_type[$list_key]}' ",ARRAY_A);
