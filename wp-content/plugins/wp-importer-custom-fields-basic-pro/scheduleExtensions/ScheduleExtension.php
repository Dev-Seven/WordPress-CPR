...
class ScheduleExtension {
	...
	public function saveEventInformationToSchedule() {
		...
		if(isset($_POST['UpdateUsing'])){
			update_option('custom_fields_importer_update_using', $_POST['UpdateUsing']);
		}
		...
		if(!is_wp_error($wpdb->insert_id)) {
			...
			if ($import_method == 'url') {
				$save_externalfile = "insert into ultimate_cfimporter_pro_external_file_schedules (schedule_id, file_url, filename) values ('{$wpdb->insert_id}', '{$_POST['fileurl']}', '{$eventKey}')";
				$wpdb->query($save_externalfile);

			} else {
				if ($import_method == 'ftp' || $import_method == 'ftps' || $import_method == 'sftp') {
					$save_ftpfile = "insert into ultimate_cfimporter_pro_ftp_schedules (schedule_id, hostname, username, password, initial_path, filename, port_no) values ('{$wpdb->insert_id}', '{$_POST['hostname']}', '{$_POST['username']}', '{$_POST['password']}', '{$_POST['initialpath']}', '{$eventKey}' , '{$_POST['portnumber']}')";
