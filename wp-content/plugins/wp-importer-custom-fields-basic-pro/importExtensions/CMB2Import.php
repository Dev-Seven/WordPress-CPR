...
class CMB2Import {
    ...
    function set_cmb2_values($header_array ,$value_array , $map, $post_id , $type){	
		...
		$this->cmb2_import_function($post_values, $post_id);

    }
    public function cmb2_import_function ($data_array,$pID) {
		$createdFields = array();
		...
		foreach ($data_array as $dkey => $dvalue) {
			...
			if($dkey == $prefix.'image'){
				$darray[$prefix.'image_id'] = $media_instance->media_handling($dvalue, $pID);
