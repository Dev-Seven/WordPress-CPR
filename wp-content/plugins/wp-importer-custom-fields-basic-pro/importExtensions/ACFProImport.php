...
class ACFProImport {
		...
		foreach($map as $key => $value){
			...
			if(!empty($csv_value)){
				...
				if(preg_match_all($pattern, $csv_value, $matches, PREG_PATTERN_ORDER)){	
					...
					if((!empty($csv_element) || $csv_element == '0') && !empty($wp_element)){
						if(is_plugin_active('advanced-custom-fields-pro/acf.php')){
							$this->acfpro_import_function($wp_element , $csv_element ,$type, $post_id,$mode,$post_values);
							...
							if(is_plugin_active('advanced-custom-fields-pro/acf.php')){
								...
								if(is_dir($acf_pluginPath)) {
									...
									$acf_instance->acf_import_function($wp_element , $csv_element ,$type, $post_id,$mode,$post_values);
							}
						}
					}		
				}
				...
				elseif(!in_array($csv_value , $header_array)){
					$wp_element= trim($key);
					if(is_plugin_active('advanced-custom-fields-pro/acf.php')){
						$this->acfpro_import_function($wp_element , $csv_value ,$type, $post_id,$mode,$post_values);
						...
						if(is_plugin_active('advanced-custom-fields/acf.php')){
							...
							if(is_dir($acf_pluginPath)) {
								...
								$acf_instance->acf_import_function($wp_element , $csv_value ,$type, $post_id,$mode,$post_values);
						}
					}
				}
				else{
	...
	function acfpro_import_function($acf_wpname_element ,$acf_csv_element , $importAs , $post_id,$mode,$imgmeta){
		global $wpdb;
		$plugin = 'acf';
					...
					if(preg_match_all('/\b(?:(?:https?|ftp|file):\/\/|www\.|ftp\.)[-A-Z0-9+&@#\/%=~_|$?!:,.]*[A-Z0-9+&@#\/%=~_|$]/i', $gallery,$matched_gallerylist,PREG_PATTERN_ORDER)){
						...
						if($ext == 'jpg' || $ext == 'jpeg' || $ext == 'png' || $ext == 'gif') {
							...
							if(!empty($img_id)) {
								...
								$get_gallery_id = ACFProImport::$media_instance->media_handling( $gallery, $post_id ,$acf_wpname_element);
								if($get_gallery_id != '') {
	...
	function acfpro_flexible_import_fuctions($data_array, $importAs, $pID,$maps,$mode) {
		global $wpdb;
		$helpers_instance = ImportHelpers::getInstance();
