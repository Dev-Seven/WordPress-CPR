...
class Dashboard {
	...
	public function fetch_LineChart_data() {
		...
		$get_list_of_imported_types = $wpdb->get_col($wpdb->prepare("select distinct( import_type ) from ultimate_cfimporter_pro_smackuci_events", array()));
		foreach($get_list_of_imported_types as $import_type) {
			$data = '';
			for($i = 0; $i <= 11; $i++) {
				$count = 0;
				$get_chart_data = $wpdb->get_results($wpdb->prepare( "select sum(created) as %s from ultimate_cfimporter_pro_smackuci_events where import_type = %s and month = %s and year = %d", $import_type, $import_type, $month[$i], $year[$i] ) );
	...
	public function fetch_PieChart_data() {
		...
		$count = 1;
		foreach($get_list_of_imported_types as $import_type) {
			$get_chart_data = $wpdb->get_results( $wpdb->prepare( "select sum(created) as %s from ultimate_cfimporter_pro_smackuci_events where import_type = %s", $import_type, $import_type ) );
			if(array_key_exists($import_type,$available_types)){
				$import_type_data = $available_types[$import_type];
			} else {
				$import_type_data = $import_type;
	...
	public function fetch_BarStackedChart_data() {
		...
		$count = 1;
		foreach($get_list_of_imported_types as $import_type) {
			$get_chart_data = $wpdb->get_results( $wpdb->prepare( "select sum(created) as created, sum(updated) as updated, sum(skipped) as skipped from ultimate_cfimporter_pro_smackuci_events where import_type = %s", $import_type, $import_type ) ); 
			if(array_key_exists($import_type,$available_types)){
				$import_type_data = $available_types[$import_type];
			} else {
				$import_type_data = $import_type;
