    ...
    public function processExtension($data) {
        ...
        if ( !empty($group_id_arr) ) {	
			foreach($group_id_arr as $groupId) {	
				$get_acf_fields = $wpdb->get_results( $wpdb->prepare( "SELECT ID, post_title, post_content, post_excerpt, post_name FROM $wpdb->posts where post_status != 'trash' AND post_parent in (%s)", array($groupId) ) );				
        ...
        if($module != 'user') {   
            ...
            if(is_plugin_active('jet-engine/jet-engine.php')){
                         ...
                         if(is_array($fields)){
                            foreach($fields['meta_fields'] as $jet_key => $jet_value){
                                ...
                                if($jet_value['type'] != 'repeater'){
                                    ...
                                    GROUP BY meta_key
                                    HAVING meta_key NOT LIKE '\_%' and meta_key NOT LIKE 'field_%' and meta_key NOT LIKE '$jet_field' and meta_key NOT LIKE 'wpcf-%' and meta_key NOT LIKE 'wpcr3_%' and meta_key NOT LIKE '%pods%' and meta_key NOT LIKE '%group_%' and meta_key NOT LIKE '%repeat_%' and meta_key NOT LIKE 'mp_%'
                                    ORDER BY meta_key" );
                                }
                ...
                WHERE p.post_type = '{$module}' AND NOT p.post_status = 'trash' AND NOT p.post_type = 'acf-field' AND NOT p.post_type = '_pods_field'
                GROUP BY meta_key
                HAVING meta_key NOT LIKE '\_%' and meta_key NOT LIKE 'field_%' and  meta_key NOT LIKE 'yachts_%' and meta_key NOT LIKE 'rank_%'and meta_key NOT LIKE 'icon_%' and meta_key NOT LIKE 'wpcf-%' and meta_key NOT LIKE 'wpcr3_%' and meta_key NOT LIKE '%pods%' and meta_key NOT LIKE '%group_%' and meta_key NOT LIKE '%repeat_%' and meta_key NOT LIKE 'mp_%'
                ORDER BY meta_key" );
            }
